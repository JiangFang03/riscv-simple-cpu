...
#endif
#include <iostream>
class DataMem_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    DataMem_api_t(VDataMem* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorQData(&(dut->io_write_data)));
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_address)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_mem_read)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_mem_write)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_read_data)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["DataMem.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    std::string vcdfile = "./generated/dm/DataMemTestMain1103304438/DataMem.vcd";
