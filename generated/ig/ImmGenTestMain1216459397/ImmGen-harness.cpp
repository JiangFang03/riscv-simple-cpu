...
#endif
#include <iostream>
class ImmGen_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    ImmGen_api_t(VImmGen* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_instruction)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_j_imm)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_u_imm)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_b_imm)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_s_imm)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_i_imm)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["ImmGen.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VImmGen* top = new VImmGen;
    std::string vcdfile = "./generated/ig/ImmGenTestMain1216459397/ImmGen.vcd";
