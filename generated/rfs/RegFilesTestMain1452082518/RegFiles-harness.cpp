...
#endif
#include <iostream>
class RegFiles_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    RegFiles_api_t(VRegFiles* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorQData(&(dut->io_w_data)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rd)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rs2)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rs1)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_reg_write)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_r_data2)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_r_data1)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["RegFiles.reset"] = 0;
    }
#if VM_TRACE
...
static RegFiles_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VRegFiles* top = new VRegFiles;
    std::string vcdfile = "./generated/rfs/RegFilesTestMain1452082518/RegFiles.vcd";
