...
#endif
#include <iostream>
class AluCtrl_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    AluCtrl_api_t(VAluCtrl* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_fun7)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_fun3)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_alu_op)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_alu_ctrl)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["AluCtrl.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VAluCtrl* top = new VAluCtrl;
    std::string vcdfile = "./generated/aluctrl/AluCtrlTestMain613385097/AluCtrl.vcd";
