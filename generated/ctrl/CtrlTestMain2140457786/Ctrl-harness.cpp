...
#endif
#include <iostream>
class Ctrl_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Ctrl_api_t(VCtrl* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_alu_op)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_branch)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_mem_write)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_mem_read)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_reg_write)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_mem_to_reg)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_alu_src)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Ctrl.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VCtrl* top = new VCtrl;
    std::string vcdfile = "./generated/ctrl/CtrlTestMain2140457786/Ctrl.vcd";
